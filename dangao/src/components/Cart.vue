...
export default {
    ...
    methods:{
        minus:function(i){
            ...
            this.$http.get("http://127.0.0.1/dangao/data/update_count.php?pid="+pid+"&count="+count);
            if(count==0){
                this.loadData();
            }
        },
        addCount(i){
            ...
            var count=this.itemList[i].count;
            var pid = this.itemList[i].product_id;
            this.$http.get("http://127.0.0.1/dangao/data/update_count.php?pid="+pid+"&count="+count).then((res)=>{
            });
        },
        loadData(){
            ...
            console.log(uid);
            if(!uid){return;}
            this.$http.get("http://127.0.0.1/dangao/data/cart.php?uid="+uid)
            .then((res)=>{
